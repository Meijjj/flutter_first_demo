...
class HomeModel {
  ...
  factory HomeModel.formJson(Map<String, dynamic> json) {
    var localNavListJson = json['localNavList'] as List;  // 转换成list数组
    List<CommonModel> localNavList = localNavListJson.map((i) => CommonModel.fromJson(i)).toList();
    var bannerListJson = json['bannerList'] as List;  // 转换成list数组
    List<CommonModel> bannerList = bannerListJson.map((i) => CommonModel.fromJson(i)).toList();
    var subNavListJson = json['subNavList'] as List;  // 转换成list数组
